...
var moveRE = regexp.MustCompile(`^([UDLR])(\d+)`)

func main() {
	fmt.Println(part1(testInput1()))
	fmt.Println(part1(testInput2()))
	fmt.Println(part1(input()))
}

func part1(input string) (int, int) {
	var path [2]map[coordinate]int
	path[0] = map[coordinate]int{}
	path[1] = map[coordinate]int{}

	paths := strings.Split(input, "\n")

	for k, p := range paths {
		moves := strings.Split(p, ",")
		path[k][coordinate{
			X: 0,
			Y: 0,
		}] = 0
	...
	for coord, _ := range path[0] {
		if _, ok := path[1][coord]; ok {
	...
	for _, v := range intersections {
		d := v.manhattanDistance(coordinate{X: 0, Y: 0})
		if d < distance {
			distance = d
		}
		//fmt.Println(path[0][v], path[1][v])
		steps := path[0][v] + path[1][v]
		if !(v.X == 0 && v.Y == 0) && steps < fewestSteps {
			fewestSteps = steps
		}
	}

	return distance, fewestSteps
}

func (c coordinate) makeMove(x string) []coordinate {
	var coords []coordinate
	var direction string
	var qty int

	p := moveRE.FindStringSubmatch(x)
	if p != nil {
		direction = p[1]
		qty, _ = strconv.Atoi(p[2])
	}

	switch direction {
	case "U":
		for y := 0; y <= qty; y++ {
			coords = append(coords, coordinate{X: c.X, Y: c.Y + y})
		}
	case "R":
		for x := 0; x <= qty; x++ {
			coords = append(coords, coordinate{X: c.X + x, Y: c.Y})
		}
	case "D":
		for y := 0; y <= qty; y++ {
			coords = append(coords, coordinate{X: c.X, Y: c.Y - y})
...
func (c coordinate) manhattanDistance(a coordinate) int {
	md := math.Abs(float64(a.X-c.X)) + math.Abs(float64(a.Y-c.Y))
	return int(md)
}
