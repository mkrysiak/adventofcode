...
func part1(adj map[Coordinate]Coordinates) int {
...
func part2(adj map[Coordinate]Coordinates) int {
...
func edgeTo(adj map[Coordinate]Coordinates) map[Coordinate]Coordinate {
	q := Coordinates{{1000, 1000}}
	marked := map[Coordinate]struct{}{
		q[0]: struct{}{},
	}

	eTo := map[Coordinate]Coordinate{}
...
func adjMap(in string) map[Coordinate]Coordinates {
	pStack := Coordinates{}
	currPos := Coordinate{1000, 1000}
	adj := map[Coordinate]Coordinates{}
	prevPos := currPos

	for _, v := range in[1 : len(in)-1] {
		switch v {
		case '|':
			currPos = pStack[len(pStack)-1]
		case '(':
			pStack = append(pStack, currPos)
		case ')':
			currPos = pStack[len(pStack)-1]
		...
		default:
			currPos.x += move[v].x
			currPos.y += move[v].y
			newPos := Coordinate{currPos.x, currPos.y}
			// Store adjacencies in both directions
			adj[prevPos] = append(adj[prevPos], newPos)
			adj[newPos] = append(adj[newPos], prevPos)
		}
		prevPos = currPos
	}
