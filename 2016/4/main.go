...
var inputRE = regexp.MustCompile(`([\w-]+)-(\d+)\[(\w+)\]`)

func main() {
	lines := strings.Split(input(), "\n")
	//lines := strings.Split("not-a-real-room-404[oarel]", "\n")
	...
	for _, line := range lines {
		...
		if m != nil {
			...
			sec := shift(m[1], sector)
			if sec == "northpole object storage" {
				sectorId = sector
			}
		}
	}
...
func checksum(s string) string {
	...
	rooms := []RoomName{}
	for k, v := range rc {
		rooms = append(rooms, RoomName{k, v})
	}

	sort.Slice(rooms, func(i, j int) bool {
		if rooms[i].count > rooms[j].count {
			return true
		}
		if rooms[i].count == rooms[j].count && rooms[i].letter < rooms[j].letter {
			return true
		}
		return false
	})
	...
	cs := []rune{}
	for _, v := range rooms {
		//fmt.Printf("%s,%d\n", string(v.letter), v.count)
		cs = append(cs, v.letter)
	}

	return string(cs[:5])
}

func shift(s string, c int) string {
	r := []rune(s)
	for i := range r {
		if r[i] == '-' {
			r[i] = ' '
